...
import { CheckCircle2, Clock, Download, FileText, RefreshCcw, Send, ShieldCheck } from "lucide-react";
import { useAuth } from "../../context/AuthContext";
import { getEmployeeReports } from "../../services/employeeAPI";
...
const EmployeeReports = () => {
  ...
  const loadReports = async () => {
    ...
    setError("");
    try {
      setReport(await getEmployeeReports(user.user_id));
    } catch (err) {
    ...
    <div className="space-y-5">
      <section className="flex flex-col gap-4 rounded-lg bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-slate-950">My portfolio reports</h1>
          <p className="mt-1 text-sm text-slate-500">
            Review activity, risk exposure, and outcomes for your assigned customers.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={() => exportApplicationsCsv(filteredApplications, "employee-applications-report.csv")}
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Assigned customers" value={loading ? "..." : summary.assignedCustomers ?? 0} detail="In your portfolio" icon={FileText} tone="bg-blue-50 text-blue-700" />
        <MetricCard label="Pending review" value={loading ? "..." : summary.pendingReview ?? 0} detail="Needs your action" icon={Clock} tone="bg-amber-50 text-amber-700" />
        <MetricCard label="Accepted" value={loading ? "..." : summary.approved ?? 0} detail={`${summary.approvalRate ?? 0}% approval rate`} icon={CheckCircle2} tone="bg-emerald-50 text-emerald-700" />
        <MetricCard label="Avg. risk score" value={loading ? "..." : summary.avgRiskScore ?? 0} detail={`${summary.sentToAdmin ?? 0} sent to admin`} icon={ShieldCheck} tone="bg-purple-50 text-purple-700" />
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={report?.statusBreakdown || []} dataKey="value" innerRadius={55} outerRadius={78} paddingAngle={3}>
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
              <Pie data={report?.riskBreakdown || []} dataKey="value" innerRadius={55} outerRadius={78} paddingAngle={3}>
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
          <h2 className="text-base font-bold text-slate-950">My application report</h2>
          <select
            value={statusFilter}
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[860px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr><td className="px-5 py-5 text-slate-500" colSpan="6">Loading applications...</td></tr>
                ...
                <tr><td className="px-5 py-5 text-slate-500" colSpan="6">No applications in your portfolio yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
